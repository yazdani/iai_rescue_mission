    ...
    (let* ((map-name "http://knowrob.org/kb/ias_semantic_map.owl#GaltelliSimMap")
      ...
      (loop for i from 0 to (length names)
            do(let* ((var (internal-string (nth i names))))
                ...
                (setf quaternion (cdr (assoc '?quaternion (car list-pose))))
                (setf translation (cdr (assoc '?translation (car list-pose))))
      ...
      (read-from-string (second tlist))
                ...
                (setf depth (cdr (assoc '?Depth (car dim-list))))
                (setf width (cdr (assoc '?Width (car dim-list))))
                (setf height (cdr (assoc '?Height (car dim-list))))
